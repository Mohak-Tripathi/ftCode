...
void getSysInfo(struct mg_rpc_request_info *ri, void *cb_arg, struct mg_rpc_frame_info *fi, struct mg_str args)
{
         ...
         c = mg_next(mgr, c))
    {
        if (c->flags & MG_F_IS_WEBSOCKET)
        {
            ...
            mg_printf_websocket_frame(c, WEBSOCKET_OP_TEXT, health, epochTime,heapSize, Freeheap_Size, min_Freeheap_Size, Fs_size,free_fs_size,cpu_freq,uptime,mgos_sys_config_get_admin_version(),mac_string);
        }
    }
